...
//! - 动态监听终端大小变化并 resize PTY（Unix 上通过可响应 shutdown 的 pending()+短睡眠轮询实现，支持干净 join，不再 forever 阻塞）
...
pub struct CompatPtySession {
    ...
    // stdout / stdin 原始字节流现在直接流式写入调用者提供的磁盘文件，实现超长会话零内存增长
...
impl CompatPtySession {
    /// 启动一个完全兼容的 PTY 会话（文件流式捕获版）
    ///
    /// `stdout_capture` / `stdin_capture` 如果提供，会在捕获线程中直接 append 原始字节到磁盘文件，
    ...
    pub fn spawn(
        ...
        stdout_capture: Option<File>,
        stdin_capture: Option<File>,
        ...
        let shutdown_out: Arc<AtomicBool> = Arc::clone(&shutdown);
        let shutdown_in: Arc<AtomicBool> = Arc::clone(&shutdown);

        // stdout 捕获线程：实时写磁盘文件（如果提供）+ 更新 ring + 事件
        let mut stdout_writer = stdout_capture.map(BufWriter::new);
            ...
            loop {
                ...
                match reader.read(&mut buffer) {
                    ...
                    Ok(n) => {
                        ...
                        // 流式写入调用者提供的原始 stdout 文件（核心：不占内存）
        ...
        // stdin 捕获线程：同样支持流式文件 + 小内存缓冲（用户输入通常很小）
        let mut stdin_writer = stdin_capture.map(BufWriter::new);
    ...
    /// 阻塞等待子进程结束，返回退出状态
    /// 现在会设置 shutdown 并 join stdout + resize 线程，确保捕获完整且线程干净退出。
    ...
    /// 移出捕获数据（仅 events + ring，原始 stdout/stdin 已在磁盘文件中）
